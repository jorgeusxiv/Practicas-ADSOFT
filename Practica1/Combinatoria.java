...
public class Combinatoria {
  ...
  private int position(int n, int k){
  ...
  public long combinaciones(int n, int k){
    ...
    //caso general
      ...
      if (cache.get(position(n, k)) == null){
        cache.put(position(n, k), combinaciones(n-1, k-1)+ combinaciones (n-1, k));
      }

      return cache.get(position(n, k));
    }
  }
